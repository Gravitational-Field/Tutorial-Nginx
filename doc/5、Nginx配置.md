...
```json
#安全问题，建议用nobody,不要用root.使用nobody用户来创建worker，从而
#user  nobody;
...
http {
	...
	# 2、普通匹配，首先通过“=”来匹配完全精确的location
        ...
        #定义服务器的默认网站根目录位置    匹配到/       则到nginx/html的，跳转到下边的三个文件
        root   html;
    ...
    location  /images/ {    
        root images ;  # 匹配nginx根目录  nginx/images
    }
    ...
    location ~*.(gif|jpg|jpeg)$ { 
        ...
        #所有静态文件直接读取硬盘
        root pic ;    # 相对于nginx的目录     /usr/local/nginx/pic下的图片文件

        #expires定义用户浏览器缓存的时间为3天，如果静态页面不常更新，可以设置更长，这样可以节省带宽和缓解服务器的压力
        expires 3d; #缓存3天   在浏览器中，而不是在服务器中
    }
